...
/*   Updated: 2023/10/02 12:53:49 by izanoni          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_close(void *v);
int		ft_key(int tecla, t_mlx *mlx, t_sprite *sprites);
void	ft_put_pix(t_mlx *mlx, int width, int height, int color);
int		ft_matrix_len(char **matrix);
void	draw_map(t_mlx *mlx, t_sprite *sprites);
int		valid_map_size(t_mlx *mlx);
void	find_player(t_mlx *mlx);
int		valid_map_name(int arg_c, char **arg_v);
int		xpm_to_img(t_mlx *mlx, void **image, char *path);
void	ft_put_sprites(t_mlx *mlx, t_sprite *sprites);
...
int	main(int argc, char **argv)
{
	...
	mlx.win_ptr = mlx_new_window(mlx.mlx_ptr, (mlx.width * PIXEL), (mlx.height * PIXEL), "Bluey!");
	find_player(&mlx);
	ft_put_sprites(&mlx, &sprites);
	draw_map (&mlx, &sprites);
...
void	ft_put_sprites(t_mlx *mlx, t_sprite *sprites)
{
	xpm_to_img(mlx, &sprites->bluey_d, BLUEY_D);
	xpm_to_img(mlx, &sprites->bluey_l, BLUEY_L);
	xpm_to_img(mlx, &sprites->bluey_r, BLUEY_R);
	xpm_to_img(mlx, &sprites->bluey_u, BLUEY_U);
	xpm_to_img(mlx, &sprites->exit_c, EXIT_C);
	xpm_to_img(mlx, &sprites->exit_o, EXIT_O);
	xpm_to_img(mlx, &sprites->tile, TILE);
	xpm_to_img(mlx, &sprites->wall, WALL);
	xpm_to_img(mlx, &sprites->cactus, CACTUS);
	xpm_to_img(mlx, &sprites->balloon, BALLOON);
	xpm_to_img(mlx, &sprites->win, WIN);
}
	...
	while (mlx->map[height] != NULL)
	{
		...
		while (mlx->map[height][width] != '\0')
		{
			if (mlx->map[height][width] == 'P')
				...
				mlx_put_image_to_window(mlx->mlx_ptr, mlx->win_ptr, sprites->bluey_d, width * PIXEL, height * PIXEL);
			if (mlx->map[height][width] == 'C')
				//ft_put_pix (mlx, width * PIXEL, height * PIXEL, 0xaaFF00FF);
				mlx_put_image_to_window(mlx->mlx_ptr, mlx->win_ptr, sprites->balloon, width * PIXEL, height * PIXEL);
			if (mlx->map[height][width] == 'E')
				//ft_put_pix (mlx, width * PIXEL, height * PIXEL, 0xaaFF0000);
				mlx_put_image_to_window(mlx->mlx_ptr, mlx->win_ptr, sprites->exit_o, width * PIXEL, height * PIXEL);
			if (mlx->map[height][width] == '0')
				//ft_put_pix (mlx, width * PIXEL, height * PIXEL, 0xaa0000FF);
				mlx_put_image_to_window(mlx->mlx_ptr, mlx->win_ptr, sprites->tile, width * PIXEL, height * PIXEL);
			else if (mlx->map[height][width] == '1')
				//ft_put_pix (mlx, width * PIXEL, height * PIXEL, 0xaa00FF00);
				mlx_put_image_to_window(mlx->mlx_ptr, mlx->win_ptr, sprites->wall, width * PIXEL, height * PIXEL);
			width++;
		}
		height++;
	}
